...
export interface SeedRunSummary {
  states: { created: number; skipped: number };
  cities: { created: number; skipped: number };
  localGovernments: { created: number; skipped: number };
  users: {
    created: number;
    skipped: number;
...
export const defaultSeedUsers: SeedUserInput[] = [
  {
    email: 'superadmin@florence.local',
    firstName: 'Super',
    lastName: 'Admin',
    password: 'Admin123!',
...
export async function runSeed(
  prisma: PrismaClient,
  users: SeedUserInput[] = defaultSeedUsers,
  ...
  if (
    !stateNameToId.has('Federal Capital Territory') &&
    stateNameToId.has('Abuja')
  ) {
  ...
  for (const [stateName, cityNames] of Object.entries(citiesData)) {
    ...
    if (!stateId) {
      summary.cities.skipped += cityNames.length;
      continue;
    }
  ...
  for (const lgas of Object.values(localGovernmentsData)) {
    for (const lgName of lgas as string[]) {
      const existingLga = await prisma.localGovernment.findFirst({
        where: { name: lgName, cityId: null },
        select: { id: true },
      });

      if (existingLga) {
        summary.localGovernments.skipped++;
        continue;
      }

      await prisma.localGovernment.create({
        data: { name: lgName, cityId: null },
      });
      summary.localGovernments.created++;
    }
  }
  ...
  for (const user of users) {
    ...
    summary.users.credentials.push({
      email: user.email,
